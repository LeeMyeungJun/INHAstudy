...
cAseLoader::cAseLoader() : m_fp(NULL)
{
}
...
void cAseLoader::ProcessMATERIAL(cMtlTex* pMtlTex)
{
	...
	do
	{
		...
		else if(IsEqual(szToken, ID_MAP_DIFFUSE))	//텍스쳐
		{
...
void cAseLoader::ProcessMAP_DIFFUSE(cMtlTex* p_Mtltex)
{
	int nLevel = 0;
	do
	{
		...
		{
			szToken = GetToken();
			p_Mtltex->SetTexture(g_pTextureManager->GetTexture(szToken));
		}
	} while (nLevel > 0);
}
...
void cAseLoader::ProcessMESH_FACE_LIST(vector<ST_PNT_VERTEX>& vecVertex, vector<D3DXVECTOR3>& vecV)
{
	...
	do
	{
		...
		else if (IsEqual(szToken, ID_MESH_FACE))
		{
			...
			GetToken();		vecVertex[nFaceIndex * 3 + 0].p = vecV[GetInteger()];	//GetToken으로 버리기
