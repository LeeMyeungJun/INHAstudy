...
MenuScene::MenuScene()
{
	...
	TitleFont = CreateFont(80, 0, 0, 0, 0, 0, 0, 0, DEFAULT_CHARSET, 0, 0, 0, 0, L"1훈새마을운동 R");
	CommonFont = CreateFont(30, 0, 0, 0, 0, 0, 0, 0, DEFAULT_CHARSET, 0, 0, 0, 0, L"빙그레체");
...
void MenuScene::UI(HDC hdc)
{
	...
	DrawText(hdc,m_Title.c_str(),m_Title.size(), &m_rc_Title, DT_TOP | DT_CENTER | DT_SINGLELINE);
	...
	DrawText(hdc, m_StartButton.c_str(), m_StartButton.size(), &m_rc_Start, DT_TOP | DT_CENTER | DT_SINGLELINE);


	/*EXIT BUTTON*/
	Rectangle(hdc, m_rc_Exit.left, m_rc_Exit.top - 10, m_rc_Exit.right, m_rc_Exit.bottom - 10);
	//폰트 start와 동일.
	DrawText(hdc, m_ExitButton.c_str(), m_ExitButton.size(), &m_rc_Exit, DT_TOP | DT_CENTER | DT_SINGLELINE);
